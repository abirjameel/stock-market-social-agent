"""Orchestrates the two halves of the daily pipeline:
  ...
  1. `generate_daily_draft` - runs the content agent, renders the image,
     archives it to Dropbox, saves a draft in Firestore, and sends it to
...
from services import dropbox_store, firestore_store, instagram_service, linkedin_service, telegram_service
...
async def generate_daily_draft() -> dict:
    ...
    upload = dropbox_store.upload_image(image_path, remote_filename=f"{today}.jpg")

    draft_id = firestore_store.create_draft(
        {
            "date": today,
            ...
            "image_dropbox_path": upload.dropbox_path,
            "image_direct_link": upload.direct_link,
...
def approve_and_publish_draft(draft_id: str) -> dict:
    ...
    local_image_path = dropbox_store.download_image(draft["image_dropbox_path"])

    results: dict = {"instagram": None, "linkedin": None, "errors": {}}

    try:
        results["instagram"] = instagram_service.publish_post(
            image_url=draft["image_direct_link"], caption=draft["instagram_caption"]
        )
